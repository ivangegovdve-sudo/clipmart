# ClipMart: Marketplace for Paperclip Team Configurations
...
**Claw Mart** sells individual AI-agent personas and skills (single-agent focus).
**ClipMart** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Paperclip-managed companies.

| Dimension | Claw Mart (Reference) | ClipMart (Our Product) |
|---|---|---|
| Unit of sale | Single persona or skill | Team blueprint (multi-agent org) |
| Buyer | Individual AI power-user | Founder / team lead spinning up an AI company |
| Install target | One agent's config files | Paperclip company (agents, projects, governance) |
| Value prop | "Skip prompt engineering" | "Skip org design — get a shipping team in minutes" |
| Price range | $0–$99 per item | $0–$499 per blueprint (+ individual add-ons) |
...
| `/about` | About ClipMart | Mission, how it works |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → ClipMart API calls Paperclip API to:
...
Running Paperclip company → "Export as Blueprint" (CLI or UI)
  → Paperclip exports:
      ...
      - Project templates
      - Skill files
  → Upload to ClipMart as new listing
...
Drawing from Claw Mart's clean aesthetic but adapted for teams:
...
### 6.2 Key Design Differentiators from Claw Mart

| Element | Claw Mart | ClipMart |
|---|---|---|
| Product card | Single avatar + description | Org chart mini-preview + agent count badge |
| Detail page | README + feature list | Interactive org chart + per-agent breakdown |
| Install flow | Drop file into workspace | One-click deploy to Paperclip company |
| Social proof | Install count | "X companies running this blueprint" |
| Preview | Screenshots | Live demo sandbox (stretch goal) |
...
| Platform fee | 10% of sale price | ClipMart's cut |
...
- [ ] Live demo sandbox
- [ ] API for programmatic publishing (a la Claw Mart's Creator API)
